...
def search_bigbasket(item_name, location_data, credentials=None):
    ...
    # Check if BigBasket credentials exist, if not generate them
    if 'BigBasket' not in credentials:
        try:
            bigbasket_credentials = get_Bigbasket_Credentials(location_data)
            credentials['BigBasket'] = bigbasket_credentials['BigBasket']
            log_debug("Generated new BigBasket credentials", "BigBasket", "INFO")
        except Exception as e:
            log_debug(f"Failed to generate BigBasket credentials: {str(e)}", "BigBasket", "ERROR")
